...
def init(surf,larg,haut) -> dict[str,Any]:
    scene = {}
    scene['surface'] = surf
    scene['larg'] = larg
    scene['haut'] = haut
    scene['les_fleurs'] = creer_des_fleurs()
    return scene

def creer_des_fleurs() -> list[dict] :
    nb_fleurs = random.randint(3,15)
    liste_fleurs = []
    for i in range(nb_fleurs) :
        fleur = creer_une_fleur()
        j = 0
        ajoute = False
        while not ajoute and j < len(liste_fleurs) :
...
def creer_une_fleur() -> dict[str:Any]:
    ...
    fleur['coords'] = (random.randint(50,550),random.randint(200,250))
